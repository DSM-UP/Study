...
시퀄라이즈에서는 N:M 관계를 표현하기 위해 belongsToMany 메서드가 있습니다.

N:M 관계 특성상 새로운 모델이 생성됩니다.
through 속성에 그 이름을 적어주면 됩니다.
새로 생성된 PostHashtag 모델에는 게시글과 해시태그의 아이디가 저장됩니다.

N:M에서는 데이터를 조회할 때 여러 단계를 거쳐야 합니다.
노드 해시태그를 사용한 게시물을 조회하는 경우를 생각해보겠습니다.

해시태그를 찾으면 그 해시태그에서 바로 getPost 메서드를 사용할 수 있습니다.
get + 모델 이름의 복수형입니다.

add + 모델 이름의 복수형 메서드도 있습니다.
두 테이블 간 N:M 관계를 추가해 줍니다.

### 7.6.4 쿼리 알아보기

쿼리는 프로미스를 반환하므로 then을 붙여 결괏값을 받을 수 있습니다.

한 가지 주의할 점은 데이터를 넣을 때 MySQL의 자료형이 아니라 시퀄라이즈 모델에 정의한 자료형대로 넣어야 한다는 것입니다.
시퀄라이즈가 알아서 MySQL 자료형으로 바꿔줍니다.
자료형이나 옵션에 부합하지 않는 데이터를 넣었을 때는 시퀄라이즈가 에러를 발생시킵니다.

where 옵션이 조건들을 나열하는 옵션입니다.
시퀄라이즈는 자바스크립트 객체를 사용해서 쿼리를 생성해야 하므로 Op.gt 같은 특수한 연산자들이 사용됩니다.
Sequelize 객체 내부의 Op 객체를 불러와 사용합니다.

시퀄라이즈의 정렬 방식입니다.
order 옵션으로 가능합니다.

update 메서드로 수정할 수 있습니다.
첫 번째 인자는 수정할 내용이고, 두 번째 인자로 수정 대상 로우를 찾는 조건입니다.
where 옵션에 조건들을 적어줍니다.
